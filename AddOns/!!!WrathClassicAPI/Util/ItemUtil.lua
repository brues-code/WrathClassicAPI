-- Backport of FrameXML/ItemUtil.lua to 3.3.5a / Lua 5.1.
--
-- Implementation differences from the modern source:
--   - Many C_Item.* calls the modern code expects are not exposed by
--     stock 3.3.5; WrathClassicAPI.dll fills those in. We shim a couple
--     of trivial entry points to the stock globals so this file loads
--     standalone, and leave the DLL-provided helpers to overwrite them.
--   - CallErrorHandler isn't in 3.3.5; route through geterrorhandler().
--   - 3.3.5 OnEvent scripts receive (self, event, ...) rather than the
--     1.12-style `event`/`arg1` globals.
--   - 3.3.5 has no concept of an item GUID and no public item-lock API,
--     so those return nil / no-op until the DLL provides backports.

C_Item = C_Item or {}
C_Item.GetItemInfo = C_Item.GetItemInfo or GetItemInfo
C_Item.LockItem = C_Item.LockItem or function() end
C_Item.UnlockItem = C_Item.UnlockItem or function() end
...
-- Default error handler routing: stock 3.3.5 has no CallErrorHandler.
...
ItemEventListener:SetScript("OnEvent", function(self, event, itemID, success)
