...
| Container | `C_Container.SwapItems`, `C_Container.MoveItem` |
...
| Friend List | `C_FriendList.GetNumFriends`, `C_FriendList.GetNumOnlineFriends`, `C_FriendList.GetFriendInfo`, `C_FriendList.GetFriendInfoByIndex`, `C_FriendList.SetFriendNotes`, `C_FriendList.SetFriendNotesByIndex`, `C_FriendList.IsFriend`, `C_FriendList.IsIgnored`, `C_FriendList.IsIgnoredByGuid` |
...
| Textures  | `Frame:CreateMaskTexture`, `Texture:AddMaskTexture`, `Texture:RemoveMaskTexture`, `Texture:GetNumMaskTextures`, `Texture:GetMaskTexture`, `Texture:SetMask` |
...
| Unit Auras | `C_UnitAuras.GetAuraDataByIndex`, `C_UnitAuras.GetBuffDataByIndex`, `C_UnitAuras.GetDebuffDataByIndex`, `C_UnitAuras.GetAuraSlots`, `C_UnitAuras.GetAuraDataBySlot`, `C_UnitAuras.UnitAuraBySlot`, `C_UnitAuras.GetUnitAuraBySpellID`, `C_UnitAuras.GetPlayerAuraBySpellID`, `C_UnitAuras.GetUnitAuras`, `C_UnitAuras.GetAuraDispelTypeColor` |
...
| Modern addon TOC directives | An addon `.toc` written for modern clients still loads. A multi-version `## Interface:` list (e.g. `## Interface: 110000, 40400, 30300`) is honored when `30300` appears anywhere in it, not just first — otherwise the addon reads as out of date. Per-file `[...]` directives are applied: `[Game]`→`Wrath`, `[Family]`→`Classic`, `[TextLocale]`→your locale, and `[AllowLoadGameType wrath]` / `[AllowLoadTextLocale <locale>]` / `[AllowLoad game]` gate whether each file loads (all conditions on a line must pass). |
| `## LoadSavedVariablesFirst: 1` | An addon whose `.toc` sets this directive has its SavedVariables restored before its Lua files run, so file-scope code (`local db = MyAddonDB`) sees the saved values immediately instead of waiting for `ADDON_LOADED`. Values the addon writes at file scope are kept. |
